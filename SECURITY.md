...
If you discover a security vulnerability in zero, please report it by:
...
chmod 600 ~/.zero/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.zero/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.zero/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run zero with a dedicated user account
...
- Keep authentication data in `~/.zero/whatsapp-auth` secure (mode 0700)
...
- Subscribe to security advisories for zero and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash zero
   sudo -u zero zero gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.zero
   chmod 600 ~/.zero/config.json
   chmod 700 ~/.zero/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.zero/logs/zero.log
   ```
...
- **Chat history is stored locally** - protect the `~/.zero` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.zero/logs/zero.log
   ```
...
- GitHub Security Advisories: https://github.com/HKUDS/zero/security/advisories
- Release Notes: https://github.com/HKUDS/zero/releases
