...
Build a custom zero channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of zero (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

zero discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `zero gateway` starts, it scans:

1. Built-in channels in `zero/channels/`
2. External packages registered under the `zero.channels` entry point group
...
```
zero-channel-webhook/
├── zero_channel_webhook/
...
```python
# zero_channel_webhook/__init__.py
from zero_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# zero_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "zero-channel-webhook"
version = "0.1.0"
dependencies = ["zero", "aiohttp"]

[project.entry-points."zero.channels"]
webhook = "zero_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
zero plugins list      # verify "Webhook" shows as "plugin"
zero onboard           # auto-adds default config for detected plugins
```

Edit `~/.zero/config.json`:
...
```bash
zero channels login <channel_name>
zero channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `zero onboard`. Override to declare your fields. |
...
Override `default_config()` so `zero onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `zero-channel-{name}` | `zero-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `zero_channel_{name}` | `zero_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/zero-channel-webhook
cd zero-channel-webhook
pip install -e .
zero plugins list    # should show "Webhook" as "plugin"
zero gateway         # test end-to-end
```
