...
Use zero programmatically — load config, run the agent, get results.
...
### `Zero.from_config(config_path?, *, workspace?)`

Create a `Zero` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.zero/config.json`. |
...
async def main():
    bot = Zero.from_config(workspace="/my/project")
