...
from zero.agent.tools.base import Tool
from zero.agent.tools.registry import ToolRegistry
from zero.providers.base import LLMResponse, ToolCallRequest
...
def _make_loop(tmp_path):
    ...
    with patch("zero.agent.loop.ContextBuilder"), \
         patch("zero.agent.loop.SessionManager"), \
         patch("zero.agent.loop.SubagentManager") as MockSubMgr:
...
def test_persist_tool_result_logs_cleanup_failures(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "zero.utils.helpers._cleanup_tool_result_buckets",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(OSError("busy")),
    )
    monkeypatch.setattr(
        "zero.utils.helpers.logger.warning",
...
def test_snip_history_drops_orphaned_tool_results_from_trimmed_slice(monkeypatch):
    ...
    monkeypatch.setattr("zero.agent.runner.estimate_prompt_tokens_chain", lambda *_args, **_kwargs: (500, None))
    token_sizes = {
        "old user": 120,
        "tool call": 120,
        "tool output": 40,
    ...
    monkeypatch.setattr(
        "zero.agent.runner.estimate_message_tokens",
...
async def test_runner_keeps_going_when_tool_result_persistence_fails():
    ...
    runner = AgentRunner(provider)
    with patch("zero.agent.runner.maybe_persist_tool_result", side_effect=RuntimeError("disk full")):
        result = await runner.run(AgentRunSpec(
            initial_messages=[{"role": "user", "content": "do task"}],
            tools=tools,
            model="test-model",
...
async def test_subagent_max_iterations_announces_existing_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zero.agent.tools.filesystem.ListDirTool.execute", fake_execute)
