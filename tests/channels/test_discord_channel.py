...
from zero.channels.discord import DiscordBotClient, DiscordChannel, DiscordConfig
...
async def test_start_returns_when_discord_dependency_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("zero.channels.discord.DISCORD_AVAILABLE", False)
...
async def test_start_handles_client_construction_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("zero.channels.discord.DiscordBotClient", _boom)
...
async def test_start_handles_client_start_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("zero.channels.discord.DiscordBotClient", _FakeDiscordClient)
...
async def test_on_message_downloads_attachments(tmp_path, monkeypatch) -> None:
    ...
    channel._handle_message = capture_handle  # type: ignore[method-assign]
    monkeypatch.setattr("zero.channels.discord.get_media_dir", lambda _name: tmp_path)
