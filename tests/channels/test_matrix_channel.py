...
from zero.channels.matrix import _build_matrix_text_content

import zero.channels.matrix as matrix_module
from zero.bus.events import OutboundMessage
from zero.bus.queue import MessageBus
from zero.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from zero.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("zero.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "zero.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("zero.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "zero.channels.matrix.asyncio.create_task", _fake_create_task
    )
