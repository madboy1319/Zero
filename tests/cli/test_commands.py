...
from zero.cli.commands import _make_provider, app
from zero.config.schema import Config
from zero.providers.openai_codex_provider import _strip_model_prefix
from zero.providers.registry import find_by_name
...
def mock_paths():
    ...
    with patch("zero.config.loader.get_config_path") as mock_cp, \
         patch("zero.config.loader.save_config") as mock_sc, \
         patch("zero.config.loader.load_config") as mock_lc, \
         patch("zero.cli.commands.get_workspace_path") as mock_ws:
...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "zero is ready" in result.stdout
...
def test_onboard_interactive_discard_does_not_save_or_create_workspace(mock_paths, monkeypatch):
    ...
    from zero.cli.onboard import OnboardResult

    monkeypatch.setattr(
        "zero.cli.onboard.run_onboard",
...
def test_onboard_uses_explicit_config_and_workspace_paths(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("zero.channels.registry.discover_all", lambda: {})
...
def test_onboard_wizard_preserves_explicit_config_in_next_steps(tmp_path, monkeypatch):
    ...
    compact_output = stripped_output.replace("\n", "")
    resolved_config = str(config_path.resolve())
    assert f'zero agent -m "Hello!" --config {resolved_config}' in compact_output
    assert f"zero gateway --config {resolved_config}" in compact_output
...
def test_openai_compat_provider_passes_model_through():
    ...
    with patch("zero.providers.openai_compat_provider.AsyncOpenAI"):
...
def test_make_provider_uses_github_copilot_backend():
    from zero.cli.commands import _make_provider
    from zero.config.schema import Config
...
def test_github_copilot_provider_strips_prefixed_model_name():
    from zero.providers.github_copilot_provider import GitHubCopilotProvider
...
async def test_github_copilot_provider_refreshes_client_api_key_before_chat():
    ...
    with patch("zero.providers.openai_compat_provider.AsyncOpenAI", return_value=mock_client):
        provider = GitHubCopilotProvider(default_model="github-copilot/gpt-5.1")
...
def test_make_provider_passes_extra_headers_to_custom_provider():
    ...
    with patch("zero.providers.openai_compat_provider.AsyncOpenAI") as mock_async_openai:
...
def mock_agent_runtime(tmp_path):
    ...
    with patch("zero.config.loader.load_config", return_value=config) as mock_load_config, \
         patch("zero.cli.commands.sync_workspace_templates") as mock_sync_templates, \
         patch("zero.cli.commands._make_provider", return_value=object()), \
         patch("zero.cli.commands._print_agent_response") as mock_print_response, \
         patch("zero.bus.queue.MessageBus"), \
         patch("zero.cron.service.CronService"), \
         patch("zero.agent.loop.AgentLoop") as mock_agent_loop_cls:
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "zero.config.loader.set_config_path",
        lambda path: seen.__setitem__("config_path", path),
    )
    monkeypatch.setattr("zero.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("zero.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("zero.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("zero.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("zero.cron.service.CronService", lambda _store: object())
    ...
    monkeypatch.setattr("zero.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("zero.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_uses_workspace_directory_for_cron_store(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("zero.config.loader.set_config_path", lambda _path: None)
    monkeypatch.setattr("zero.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr("zero.cli.commands.sync_workspace_templates", lambda _path: None)
    monkeypatch.setattr("zero.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("zero.bus.queue.MessageBus", lambda: object())
    ...
    monkeypatch.setattr("zero.cron.service.CronService", _FakeCron)
    monkeypatch.setattr("zero.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("zero.cli.commands._print_agent_response", lambda *_args, **_kwargs: None)
...
def test_agent_workspace_override_does_not_migrate_legacy_cron(
    ...
    monkeypatch.setattr("zero.cli.commands._make_provider", lambda _config: object())
    monkeypatch.setattr("zero.bus.queue.MessageBus", lambda: object())
    monkeypatch.setattr("zero.config.paths.get_cron_dir", lambda: legacy_dir)
...
def _patch_cli_command_runtime(
    ...
    *,
    ...
    monkeypatch.setattr("zero.config.loader.load_config", lambda _path=None: config)
    monkeypatch.setattr(
        "zero.cli.commands.sync_workspace_templates",
        sync_templates or (lambda _path: None),
    )
    monkeypatch.setattr(
        "zero.cli.commands._make_provider",
        make_provider or (lambda _config: object()),
    )

    if message_bus is not None:
        monkeypatch.setattr("zero.bus.queue.MessageBus", message_bus)
    if session_manager is not None:
        monkeypatch.setattr("zero.session.manager.SessionManager", session_manager)
    if cron_service is not None:
        monkeypatch.setattr("zero.cron.service.CronService", cron_service)
    if get_cron_dir is not None:
        monkeypatch.setattr("zero.config.paths.get_cron_dir", get_cron_dir)
...
def _patch_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    ...
    monkeypatch.setattr("zero.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("zero.api.server.create_app", _fake_create_app)
...
def test_migrate_cron_store_moves_legacy_file(tmp_path: Path) -> None:
    ...
    from zero.cli.commands import _migrate_cron_store
    ...
    with patch("zero.config.paths.get_cron_dir", return_value=legacy_dir):
