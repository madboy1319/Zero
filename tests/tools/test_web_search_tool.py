...
from zero.agent.tools.web import WebSearchTool
from zero.config.schema import WebSearchConfig
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "Zero", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="zero", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("zero.agent.tools.web.DDGS", MockDDGS, raising=False)
    import zero.agent.tools.web as web_mod
