...
from zero.agent.memory import MemoryStore
from zero.utils.prompt_templates import render_template
from zero.agent.skills import SkillsLoader
from zero.utils.helpers import build_assistant_message, detect_image_mime
...
class ContextBuilder:
    ...
    def build_system_prompt(
        ...
        if user_profile:
            # Only include non-empty fields to keep the prompt clean
            profile_lines = []
            for k, v in user_profile.items():
                if v and v != "neutral" and v is not False:
    ...
    def build_messages(
        ...
        messages = [
            {"role": "system", "content": self.build_system_prompt(skill_names, user_profile)},
            *history,
        ]
