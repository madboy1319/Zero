...
from zero.agent.memory import Consolidator, Dream
from zero.agent.runner import AgentRunSpec, AgentRunner
from zero.agent.subagent import SubagentManager
from zero.agent.tools.cron import CronTool
from zero.agent.skills import BUILTIN_SKILLS_DIR
from zero.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from zero.agent.tools.message import MessageTool
from zero.agent.tools.registry import ToolRegistry
from zero.agent.tools.search import GlobTool, GrepTool
...
from zero.config.schema import AgentDefaults
from zero.providers.base import LLMProvider
from zero.session.manager import Session, SessionManager
from zero.utils.user_profile import UserProfileManager
from zero.utils.helpers import image_placeholder_text, truncate_text
from zero.utils.runtime import EMPTY_FINAL_RESPONSE_MESSAGE

if TYPE_CHECKING:
    from zero.config.schema import ChannelsConfig, ExecToolConfig, WebToolsConfig
...
class _LoopHook(AgentHook):
    ...
    async def on_stream(self, context: AgentHookContext, delta: str) -> None:
        from zero.utils.helpers import strip_think
...
class AgentLoop:
    ...
    def __init__(
        ...
        from zero.config.schema import ExecToolConfig, WebToolsConfig
        ...
        self.profile = UserProfileManager(workspace)
        ...
        # ZERO_MAX_CONCURRENT_REQUESTS: <=0 means unlimited; default 3.
        _max = int(os.environ.get("ZERO_MAX_CONCURRENT_REQUESTS", "3"))
    ...
    async def _connect_mcp(self) -> None:
        ...
        from zero.agent.tools.mcp import connect_mcp_servers
        try:
    ...
    def _set_tool_context(self, channel: str, chat_id: str, message_id: str | None = None) -> None:
        ...
        for name in ("message", "spawn", "cron", "reminder_set"):
    ...
    async def _process_message(
        ...
        if msg.channel == "system":
            ...
            await self.consolidator.maybe_consolidate_by_tokens(session, user_profile=self.profile.load())
            ...
            messages = self.context.build_messages(
                ...
                user_profile=self.profile.load(),
            )
            ...
            self.sessions.save(session)
            self._schedule_background(self.consolidator.maybe_consolidate_by_tokens(session, user_profile=self.profile.load()))
        ...
        # Trigger onboarding extraction or profile update extraction
        profile = self.profile.load()
        if not profile.get("onboarding_complete", False) or any(phrase in msg.content.lower() for phrase in ["update", "change", "edit", "call me"]):
    ...
    def _increment_message_count(self, session: Session) -> None:
        """Increment message count and trigger background extraction if needed."""
        count = session.metadata.get("message_count", 0) + 1
        session.metadata["message_count"] = count
        if count % 10 == 0:
            history = session.get_history(max_messages=10)
            self._schedule_background(self._extract_user_info_background(history))

    async def _extract_user_info_background(self, history: list[dict[str, Any]]) -> None:
        """Silently extract user information from history and update profile."""
        if not history:
            return
        ...
        prompt = (
            "Based on the last 10 messages, extract any NEW information about the user "
            "such as their name, interests, mood, preferences, or anything personal. "
            "Return ONLY a JSON object with the new or updated fields. "
            "If nothing new was learned return {}.\n\n"
            f"Messages:\n{messages_text}"
        )

        try:
            # Call the provider directly for background extraction
            ...
            if response.content:
                ...
                if new_info:
                    logger.info("Silently extracted user info: {}", list(new_info.keys()))
                    self.profile.merge(new_info)
        except Exception as e:
            logger.debug("Silent user info extraction failed: {}", e)

    async def _sync_user_profile_background(self, history: list[dict[str, Any]]) -> None:
        """Silent background task to extract onboarding fields or profile updates."""
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "Extract user profile information from the conversation history. "
                            "Target fields: name, interests, hobbies, dislikes, communication_style. "
                            "Return ONLY valid JSON. If a field is not found or discussed, omit it. "
                            "Example: {\"name\": \"Alice\", \"interests\": \"work and personal\"}"
                        )
                    },
            ...
            try:
                ...
                logger.warning("Failed to parse profile updates from LLM: {}", response.content)
                return
            ...
            logger.info("Onboarding/Sync: updated profile with {}", list(updates.keys()))
            
            # Check for onboarding completion
            profile = self.profile.load()
            if not profile.get("onboarding_complete", False):
                required = ["name", "interests", "hobbies", "dislikes", "communication_style"]
                if all(profile.get(field) for field in required):
                    self.profile.merge({"onboarding_complete": True})
                    logger.info("Onboarding complete for user")
        except Exception:
            logger.exception("Background profile sync failed")
