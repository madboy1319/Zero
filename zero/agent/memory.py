...
from zero.utils.helpers import ensure_dir, estimate_message_tokens, estimate_prompt_tokens_chain, strip_think

from zero.agent.runner import AgentRunSpec, AgentRunner
from zero.agent.tools.registry import ToolRegistry
from zero.utils.gitstore import GitStore
...
class Consolidator:
    ...
    async def maybe_consolidate_by_tokens(
        ...
        async with lock:
            ...
            target = budget // 2
            estimated, source = self.estimate_session_prompt_tokens(session, user_profile)
            if estimated <= 0:
                return
            if estimated < budget:
                logger.debug(
...
class Dream:
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        from zero.agent.tools.filesystem import EditFileTool, ReadFileTool
