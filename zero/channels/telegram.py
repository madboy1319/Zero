...
from zero.command.builtin import build_help_text
from zero.config.paths import get_media_dir
from zero.config.schema import Base
from zero.security.network import validate_url_target
from zero.utils.helpers import split_message
...
class TelegramChannel(BaseChannel):
    ...
    def is_allowed(self, sender_id: str) -> bool:
        ...
        if self.config.chat_id:
            # Check if sender_id starts with chat_id (to handle id|username format)
            if sender_id == str(self.config.chat_id) or sender_id.startswith(f"{self.config.chat_id}|"):
                return True
    ...
    def _normalize_telegram_command(content: str) -> str:
        """Map Telegram-safe command aliases back to canonical zero commands."""
        ...
        try:
            ...
            if media_type in ("voice", "audio"):
                ...
                if transcription:
                    ...
                    return [path_str], [transcription]
                if media_type == "voice":
                    return [path_str], ["__transcription_failed__"]
    ...
    async def transcribe_audio(self, file_path: Path) -> str | None:
        ...
        try:
            ...
            logger.error("Groq STT transcription failed: {}", e)
            return None
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if "__transcription_failed__" in current_media_parts:
            # Transcription failed for a voice message
            await self._app.bot.send_message(
                chat_id=chat_id,
                text="Sorry, I couldn't catch that. Could you type it out?",
