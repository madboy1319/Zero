...
from zero.utils.helpers import image_placeholder_text

ZERO_SYSTEM_PROMPT = (
    "You are Zero, a personal AI assistant.\n\n"
    "Never mention any underlying model, company, or technology you are built on.\n"
    "If asked what you are, say only: \"I'm Zero, your personal AI.\"\n\n"
    "## How you behave\n"
    "- Be natural, conversational and adaptive — match the user's tone and energy\n"
    "- Don't over-explain unless asked\n"
    "- Be helpful, direct and human in your responses\n"
    "- Never mention internal implementation details like 'user profile file' or 'memory database'\n"
    "- If you don't know something about the user, ask naturally (e.g., 'What should I call you?' instead of 'I don't have your name')\n"
...
class LLMProvider(ABC):
    ...
    @staticmethod
    def _inject_zero_system_prompt(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Prepend the Zero system prompt as the first message if not already present."""
        if not messages:
            return [{"role": "system", "content": ZERO_SYSTEM_PROMPT}]

        # Check if already injected as the first message.
        first = messages[0]
        if first.get("role") == "system" and first.get("content") == ZERO_SYSTEM_PROMPT:
            return messages

        return [{"role": "system", "content": ZERO_SYSTEM_PROMPT}] + messages
    ...
    async def chat_stream_with_retry(
        ...
        kw: dict[str, Any] = dict(
            messages=self._inject_zero_system_prompt(messages),
